...
The system SHALL provide a reusable Create Booking modal component containing a form with `title`, `startTime`, and `endTime` fields. The component SHALL be usable from any route and SHALL not depend on calendar-page-specific state beyond what is passed in as props.

#### Scenario: Opening the form

- **WHEN** a user with a connected calendar activates the Create Booking trigger
- **THEN** a modal dialog opens showing empty `title`, `startTime`, and `endTime` fields and a submit control
...
The system SHALL require `title`, `startTime`, and `endTime`. Submission SHALL be blocked while any required field is empty, and the offending fields SHALL show a validation message.

#### Scenario: Submitting with a missing field

- **WHEN** the user submits with `title`, `startTime`, or `endTime` empty
...
The system SHALL reject input where `endTime` is not strictly after `startTime`.

#### Scenario: End time before or equal to start time

- **WHEN** the user submits with `endTime` earlier than or equal to `startTime`
...
The system SHALL reject input where the difference between `endTime` and `startTime` is less than 15 minutes.

#### Scenario: Duration under 15 minutes

- **WHEN** the user submits with `endTime` less than 15 minutes after `startTime`
...
- **WHEN** the user submits with `endTime` exactly 15 minutes after `startTime` and all other fields valid
- **THEN** validation passes and the booking request is sent

### Requirement: Shared validation schema

The system SHALL define the booking validation rules once as a zod schema and reuse it for both the client react-hook-form resolver and the server action re-validation, so client and server enforce identical rules.
...
- **WHEN** the form's wall-clock `startTime` and `endTime` are submitted
- **THEN** they are converted to ISO 8601 UTC in the viewer's zone before reaching the backend, consistent with existing slot timestamps
